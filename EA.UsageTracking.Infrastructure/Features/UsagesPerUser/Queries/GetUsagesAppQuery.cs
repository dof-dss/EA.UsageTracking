...
{
    public class GetUsagesAppQuery : IRequest<Result<PagedResponse<UsageItemDTO>>>
    {
    ...
    public class GetUsagesAppQueryHandler : RequestHandler<GetUsagesAppQuery, Result<PagedResponse<UsageItemDTO>>>
    {
        private readonly IUriService _uriService;
        private readonly GetUsagesAppValidator _validator;
        private readonly UsageTrackingContext _usageTrackingContext;
        private readonly IMapper _mapper;
        private readonly HttpContext _httpContext;

        public GetUsagesAppQueryHandler(UsageTrackingContext usageTrackingContext, IMapper mapper,
            IUriService uriService, IHttpContextAccessor httpContextAccessor)
        {
            _uriService = uriService;
            _usageTrackingContext = usageTrackingContext;
            _mapper = mapper;
            _validator = new GetUsagesAppValidator();
            _httpContext = httpContextAccessor.HttpContext;
        }

        protected override Result<PagedResponse<UsageItemDTO>> Handle(GetUsagesAppQuery message)
        {
            ...
            var pagination = _mapper.Map<PaginationDetails>(message)
                .WithTotal(_usageTrackingContext.UsageItems
                    .IgnoreQueryFilters()
                    .Count(i => i.ApplicationUserId == userId && i.ApplicationId == message.ApplicationId));
            ...
            var query = _usageTrackingContext.UsageItems
                ...
                .Where(i => i.ApplicationUserId == userId && i.ApplicationId == message.ApplicationId);
        ...
        private Result Validate(GetUsagesAppQuery request)
        {
